...
def mapplots(column,cmap,bound,axs,gw_s, 
             waterbodies,waterways,cities,ND, marker='v', 
             normalize=True, ls=12,fs=14, ms=20, 
             legend=False,legend_title=None, legend_kwds=None, 
             categorical=False ):
    
    
    gw=gw_s.plot(ax=axs,figsize=(10, 10),column=column, markersize=ms,
               marker=marker, facecolor="None",cmap=cmap, zorder=1, categorical=categorical,
               legend=True, legend_kwds=legend_kwds)
    
    waterbodies.plot( ax=axs, alpha=0.8, color='b', linewidth=0.8, zorder=1)
    ...
    gw.add_artist(scalebar)
    gw.tick_params(axis='y', which='major', labelsize=ls, rotation=90)
    gw.tick_params(axis='x', which='major', labelsize=ls, rotation=0)
    startx, endx = gw.get_xlim()
    starty, endy = gw.get_ylim()
    ...
    arrx=endx- endx*0.002
    arry=endy-endy*0.0040
    gw.text(x=arrx-arrx*0.0001, y=arry, s='N', fontsize=fs,alpha=0.8)
    gw.arrow(arrx, arry-arry*0.002, 0, 10000, length_includes_head=True,
              head_width=8000, head_length=20000, overhang=.2, ec="k",facecolor='k', alpha=0.4)

    #Basemap
    #cx.providers.OpenStreetMap.Mapnik
    #cx.add_basemap(ax=NS,  crs=ND.crs.to_string(), 
    #               source=cx.providers.Esri.WorldTerrain,
    #               alpha=1,attribution=False,zorder=0)


    return axs

def mapplots2(column,cmap,bound,axs,gw_s, 
             waterbodies,waterways,cities,ND, marker='v', 
             normalize=True, ls=12,fs=14, ms=20):
    
    gw=gw_s.plot(ax=axs,figsize=(10, 10),column=column, markersize=ms,
               marker=marker, facecolor="None",cmap=cmap, zorder=1)
    waterbodies.plot( ax=axs, alpha=0.8, color='b', linewidth=0.8, zorder=1)
    waterways.plot( ax=axs, alpha=0.3, color='b', linewidth=.5,zorder=2)
    cit=cities.plot( ax=gw, alpha=1, color='k',markersize=5,zorder=2)
    NS=ND.boundary.plot( ax=axs, alpha=0.3, edgecolor='k', linewidth=1,
    ...
    if column =='r2' or column == 'r2_2':
        cbar.ax.set_xlabel('R$^2$', fontsize=fs)
    elif column == 'NSE_2':
        cbar.ax.set_xlabel('NSE', fontsize=fs)
    elif column == 'diffr2':
        cbar.ax.set_xlabel('$\Delta$R$^2$', fontsize=fs)
    elif column == 'diffNSE':
        cbar.ax.set_xlabel('$\Delta$NSE', fontsize=fs)
        
    elif column =='HadGEM_WRF361H_scores' :
            cbar.ax.set_xlabel('R$^2$ (HadGEM WRF361H)', fontsize=fs)    
    elif column =='ECE_RACMO_r12_scores' :
            cbar.ax.set_xlabel('R$^2$ (ECE RACMO)', fontsize=fs)
...
def mapplotsproj(column,bound,axs,gw_s, cities=None, waterbodies=None,waterways=None,ND=None, 
                color=None,cmap=None,marker='v',scheme=None,
               classification_kwds=None,zorder=3, legend_title=None, legend_kwds=None, label=None,
                 ls=14,fs=16, ms=20):
    
    gw=gw_s.plot(ax=axs,figsize=(10, 10),column=column, markersize=ms,scheme=scheme,
    ...
    if isinstance(ND,gpd.geodataframe.GeoDataFrame):
        NS=ND.boundary.plot( ax=axs, alpha=0.25, edgecolor='k', linewidth=1, zorder=1)    
        
    if isinstance(cities,gpd.geodataframe.GeoDataFrame):
        cities.plot( ax=axs, alpha=0.5, color='k',zorder=2, markersize=5) 
        for x, y, label in zip(cities.geometry.x, cities.geometry.y, cities.name):
            NS.annotate(label, xy=(x, y), xytext=(-15, -10), textcoords="offset points", fontsize=10, color="k", alpha=0.9)
...
def mapplots4(column,cmap,bound,axs,gw_s, 
             waterbodies,waterways,cities,ND, marker='v', 
             normalize=True, ls=12,fs=14, ms=20, 
             legend=False,legend_title=None, legend_kwds=None, 
             categorical=False ):
    ...
    gw=gw_s.plot(ax=axs,figsize=(10, 10),column=column, markersize=ms,
               marker=marker,scheme='QUANTILES', facecolor="None",cmap=cmap, zorder=1, 
               legend=True,legend_kwds=legend_kwds)
    ...
    for x, y, label in zip(cities.geometry.x, cities.geometry.y, cities.name):
        NS.annotate(label, xy=(x, y), xytext=(-15, 5), textcoords="offset points", fontsize=14, color="k", alpha=0.9)
